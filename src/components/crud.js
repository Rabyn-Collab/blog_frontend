...
const CrudForm = () => {
  ...
  const blogSchema = Yup.object().shape({
    ...
    detail: Yup.string().min(120, 'Too short').max(2000, 'Too Long').required('detail is required'),
  });
    ...
    onSubmit: async (val, { resetForm }) => {
      ...
      try {
        ...
        if (type === 'create') {
          ...
          toast.success('blog add succesfully');
          nav('/', { replace: true });
    ...
    <div>
      ...
      <form onSubmit={formik.handleSubmit} className='flex justify-center mt-3 items-center max-h-2xl '>
        <div className='w-[40%] shadow-2xl bg-white p-4 space-y-4 lg:w-[90%] xl:w-[90%]'>
          ...
          <div>
            <button
              disabled={isLoading || isLoad ? true : false}
              type='submit'
